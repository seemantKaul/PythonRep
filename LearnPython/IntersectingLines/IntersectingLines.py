def crossedWires(line1, line2, option):
    # print (line1)
    if option == "distance":
        print("computing distance")
        line1Coordinates = plotPoints(line1)
        line2Coordinates = plotPoints(line2)

        # print("Line 1 coordinates:")
        # for a in line1Coordinates:
        #   print (a)

        # print("Line 2 coordinates:")
        # for a in line2Coordinates:
        #    print (a)

        intersections = findIntersections(line1Coordinates, line2Coordinates)
        print("Intersections")
        # for a in intersections:
        #    print (a)
        intersectionList = list()
        for point in intersections:
            intersectionList.append(computeDistance(STARTPOINT, point))

        # print (intersectionList)
        return min(intersectionList)
    elif option == "steps":
        line1Coordinates = trackPoints(line1)
        line2Coordinates = trackPoints(line2)
        ...
        intersections = trackIntersections(line1Coordinates, line2Coordinates)
        print("Intersections")
        # for a in intersections:
        #    print (a)
        intersectionList = list()
        for point in intersections:
            intersectionList.append(
                computeSteps(point, line1Coordinates, line2Coordinates))
        # print (intersectionList)
        return min(intersectionList)


def computeDistance(fromPoint, toPoint):
    return int(
        abs(fromPoint.getX() - toPoint.getX()) +
        abs(fromPoint.getY() - toPoint.getY()))


def computeSteps(
        toPoint, line1Coordinates,
        line2Coordinates):  # returns a sum of steps for a point for both lines
    distance1 = line1Coordinates.index(toPoint) + 1
    distance2 = line2Coordinates.index(toPoint) + 1
...
def findIntersections(line1Coordinates,
                      line2Coordinates):  # returns a set of intersection Points
    return line1Coordinates.intersection(line2Coordinates)


def trackIntersections(
        line1Coordinates,
        line2Coordinates):  # returns a set of intersection Points
    setLine1 = set((line1Coordinates))
    setLine2 = set((line2Coordinates))
    return setLine1.intersection(setLine2)
    # for point1 in line1Coordinates:
    #    if line2Coordinates.index(point1) !=None:
    #        setIntersections.add(point1)


def plotPoints(line):  # returns a set of Points
    startPoint = STARTPOINT
    setPoints = set(())
    for d in line:  # traverse through all the directions

        tupleOutput = getAllPoints(startPoint, d)
        setPoints.update(tupleOutput[0])
        startPoint = tupleOutput[1]
    return setPoints


def trackPoints(line):  # returns a List of Points in order
    startPoint = STARTPOINT
    listPoints = list()
    # print(type(setPoints))
    for d in line:  # traverse through all the directions
        tupleOutput = trackAllPoints(startPoint, d)
        listPoints.extend(tupleOutput[0])
        startPoint = tupleOutput[1]
    # for p in listPoints:
    # print(p)
    return listPoints
...
def getAllPoints(startPoint,
                 direction):  # returns a set of all coordinates for this move
    # print(direction)
    vector = direction[0]
    ...
    # setPoints.clear()
    if vector == 'U':
        endpoint = Point(startPoint.getX(), startPoint.getY() + scalar)
        for i in range(startPoint.getY(), endpoint.getY()):
            setPoints.add(Point(startPoint.getX(), i + 1))

    if vector == 'D':
        endpoint = Point(startPoint.getX(), startPoint.getY() - scalar)
        for i in range(endpoint.getY(), startPoint.getY()):
            setPoints.add(Point(startPoint.getX(), i))

    if vector == 'R':
        endpoint = Point(startPoint.getX() + scalar, startPoint.getY())
        for i in range(startPoint.getX(), endpoint.getX()):
            setPoints.add(Point(i + 1, startPoint.getY()))

    if vector == 'L':
        endpoint = Point(startPoint.getX() - scalar, startPoint.getY())
        for i in range(endpoint.getX(), startPoint.getX()):
            setPoints.add(Point(i, startPoint.getY()))

    return (setPoints, endpoint)


def trackAllPoints(
        startPoint,
        direction):  # returns a List of all coordinates for this move in order
    vector = direction[0]
    scalar = int(direction[1:])
    # print(f"vector:{vector}, scalar:{scalar}")
    listPoints = list()
    if vector == 'U':
        endpoint = Point(startPoint.getX(), startPoint.getY() + scalar)
        for i in range(startPoint.getY(), endpoint.getY()):
            listPoints.append(Point(startPoint.getX(), i + 1))

    if vector == 'D':
        endpoint = Point(startPoint.getX(), startPoint.getY() - scalar)
        # print(endpoint)
        for i in range(startPoint.getY() - 1, endpoint.getY() - 1, -1):
            listPoints.append(Point(startPoint.getX(), i))

    if vector == 'R':
        endpoint = Point(startPoint.getX() + scalar, startPoint.getY())
        for i in range(startPoint.getX(), endpoint.getX()):
            listPoints.append(Point(i + 1, startPoint.getY()))

    if vector == 'L':
        endpoint = Point(startPoint.getX() - scalar, startPoint.getY())
        for i in range(endpoint.getX(), startPoint.getX()):
            listPoints.append(Point(i, startPoint.getY()))
    # for i in listPoints:
    #    print(i)
    # print(f"endpoint={endpoint}")
    return (listPoints, endpoint)
...
if __name__ == "__main__":
    ...
    # line1 = str("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51").split(',')
    # line2 = str("U98,R91,D20,R16,D67,R40,U7,R15,U6,R7").split(',')
    line1 = str(
    ...
    print(crossedWires(line1, line2, "steps"))
